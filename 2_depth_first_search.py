...
class DFSOnAVLTree:
    ...
    def __depth_first_search__(self, node):
        visited = []

        def __dfs__(root, visit):
            if root is None:
                return []
            else:
                visit.append(root.data)
                __dfs__(root.left, visit)
                __dfs__(root.right, visit)
                return visit

        result = __dfs__(node, visited)
...
if __name__ == '__main__':
    ...
    print('Dreath First Search:')
    result = avl.__depth_first_search__(root)
    print(f'Traversed in  Depth First Search: {result}')
